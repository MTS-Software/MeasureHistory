...
import com.gretha.shared.util.jfxutils.EventHandlerManager;
...
public class ChartZoomManager {
	...
	/**
	 * Construct a new ChartZoomManager. See {@link ChartZoomManager} documentation
	 * for normal usage.
	 *
	 * @param chartPane
	...
	/**
	 * Sets the {@link AxisConstraintStrategy} to use for mouse drag events, which
	 * determines which axis is allowed for zooming. The default implementation is
	...
	/**
	 * Sets the {@link AxisConstraintStrategy} to use for mouse wheel events, which
	 * determines which axis is allowed for zooming. The default implementation is
	...
	/**
	 * Sets the mouse filter for starting the zoom action. If the filter consumes
	...
	/**
	 * Stop managing zoom management by removing all event handlers and bindings,
	 * and hiding the rectangle.
	 */
	public void stop() {
	...
	private void onMouseReleased() {
		...
		Rectangle2D zoomWindow = chartInfo.getDataCoordinates(selectRect.getTranslateX(), selectRect.getTranslateY(),
				rectX.get(), rectY.get());
		...
		public void handle(ScrollEvent event) {
			if (event.isShortcutDown()) {
				...
				// If we are allowing mouse wheel zooming
						...
						// If we aren't between SCROLL_STARTED and
						// SCROLL_FINISHED
						!ignoring &&
						// inertia from non-wheel gestures might have touch
						// count of
						// 0
						!event.isInertia() &&
					...
					// If we are are doing a zoom animation, stop it. Also of
					// note
					// is that we don't zoom the
					// mouse wheel zooming. Because the mouse wheel can "fly"
					// and
					// generate a lot of events,
					// animation doesn't work well. Plus, as the mouse wheel
					// changes
					// the view a small amount in
					// a predictable way, it "looks like" an animation when you
					// roll
					// it.
					// We might experiment with mouse wheel zoom animation in
					// the
					// future, though.
					zoomAnimation.stop();

					// At this point we are a mouse wheel event, based on
					// everything
					// I've read
					...
					// Are we zooming in or out, based on the direction of the
					// roll
					double direction = -Math.signum(event.getDeltaY());

					// TODO: Do we need to handle "continuous" scroll wheels
					// that
